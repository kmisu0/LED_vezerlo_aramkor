...
#ifndef(STACK_USE_GENERIC_TCP_SERVER_EXAMPLE)
...
void GenericTCPServer(void)
{
	...
	static TCP_SOCKET	MySocket;				//TCP_SOCKET típusú mysocet változó ami egy byte
	...
	switch(TCPServerState)
	{
		case SM_HOME:
			...
			//Ha a szerver státusza SM_HOME, megnyitja a TCPportot, mint szerver.
		...
		case SM_LISTENING:
			...
			wMaxGet = TCPIsGetReady(MySocket);	// Get TCP RX FIFO byte count//Hány byte olvasható az RX bufferb?l
			wMaxPut = TCPIsPutReady(MySocket);	// Get TCP TX FIFO free space//Hány byte írható a TX bufferbe

			// Make sure we don't take more bytes out of the RX FIFO than we can put into the TX FIFO
			// Ha az TX-be írható byte-ok száma kisebb mint a beolvasható fileoké, akkor az írhatóak száma legyen egyenl? a beolvashatóak számával
			if(wMaxPut < wMaxGet)
				wMaxGet = wMaxPut;
